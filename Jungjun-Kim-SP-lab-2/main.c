...
void find_by_first_name(struct employee employees[], int num_employees) {
    ...
    for (int i = 0; i < num_employees; i++) {
        if (strcasecmp(employees[i].first_name, first_name) == 0) {
...
// void print_lowest_salaries(struct employee employees[], int num_employees) {
//     int m;
//     printf("Enter the number of employees to display with the lowest salaries: ");
//     scanf("%d", &m);
...
//     qsort(temp, num_employees, sizeof(struct employee), compare_salaries);
...
int handle_user_choice(int choice, struct employee employees[], int* num_employees) {
    switch (choice) {
        ...
        case 8:
            ...
            break;
        case 9:
            find_by_first_name(employees, *num_employees);
            break;
        default:
